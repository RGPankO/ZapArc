...
import { usePaymentPlans, usePaymentStatus, useProcessSubscription, useProcessPurchase } from '../hooks';
import { PaymentConfirmationModal, PaymentErrorModal, PaymentSuccessModal } from '../components';
...
export function PremiumScreen(): React.JSX.Element {
  ...
  const { data: plans = [], isLoading: plansLoading, error: plansError } = usePaymentPlans();
  ...
  const isProcessing = processSubscription.isPending || processPurchase.isPending;

  const handleSubscriptionPurchase = (): void => {
    if (!selectedPlan) return;

    processSubscription.mutate(selectedPlan.id, {
      onSuccess: () => {
        setShowConfirmationModal(false);
        setShowSuccessModal(true);
      },
      onError: (err) => {
        setPaymentError(err.message || 'Failed to process subscription');
        setShowConfirmationModal(false);
  ...
  const handleOneTimePurchase = (): void => {
    if (!selectedPlan) return;

    processPurchase.mutate(selectedPlan.id, {
      onSuccess: () => {
        setShowConfirmationModal(false);
        setShowSuccessModal(true);
      },
        ...
        <View style={styles.content}>
          ...
          {plansError && (
            <View style={styles.errorBanner}>
              <Text style={styles.errorBannerText}>{plansError.message || 'Failed to load plans'}</Text>
            </View>
          )}
